...
from cloud_client import download_file, upload_file
...
async def on_ready():
    ...
    try:
        download_file("memory.json", "memory.json")
        download_file("user.json", "user.json")
        download_file("server.json", "server.json")
        print("[Cloud Sync] Downloaded latest JSON backups.")
    except Exception as e:
...
async def periodic_autosave():
    ...
    while not bot.is_closed():
        ...
        try:
            ...
            server_tracker.export_json(SERVER_TRACKER_BACKUP)
            json_memory.save()
            print("[Autosave] Local backups saved.")

            # Upload to cloud
            upload_file("memory.json", "memory.json")
            upload_file("user.json", "user.json")
            upload_file("server.json", "server.json")
            print("[Cloud Sync] Uploaded JSON backups.")
        except Exception as e:
