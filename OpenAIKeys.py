...
async def update_auto_relationship(guild: discord.Guild, user_member: discord.Member, new_relationship: str):
    ...
    if monika_member:
        incorrect_role = "The literature Club's Boyfriend"
        incorrect = discord.utils.get(guild.roles, name=incorrect_role)
        if incorrect not in monika_member.roles:
            await monika_member.remove_roles(incorrect, reason="incorrect role")
        return
